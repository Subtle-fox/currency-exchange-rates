...
import com.subtlefox.currencyrates.domain.Rates
...
fun CurrencyRate.rebase(currency: CurrencyRate) = copy(
    rate = this.rate.divide(
        currency.rate,
        mc
    )
)

class RatesToValuesMapper
@Inject constructor() : BiFunction<CurrencyValue, Rates, CurrencyValues> {
    ...
    override fun apply(request: CurrencyValue, rates: Rates): CurrencyValues {
        ...
        rateList.forEachIndexed { idx, value ->
            ...
            converted[convIdx] = CurrencyValue(
                value.iso,
                value.rate.multiply(request.value, mc)
            )
        }
