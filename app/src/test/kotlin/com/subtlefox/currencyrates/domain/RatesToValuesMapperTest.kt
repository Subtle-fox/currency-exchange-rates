...
import com.subtlefox.currencyrates.domain.implementation.RatesToValuesMapper
import com.subtlefox.currencyrates.domain.implementation.rebase
import org.junit.Assert.*
import org.junit.Test
import java.math.BigDecimal
import java.math.RoundingMode

class RatesToValuesMapperTest {
    val mapper = RatesToValuesMapper()
    ...
    fun `should return empty list if no base currency found`() {
        ...
        val rebased = mapper.rebaseList(valueOf("EUR"), rates)
    ...
    fun `should return same list if base equals to request`() {
        ...
        val rebased = mapper.rebaseList(valueOf("USD"), rates)
    ...
    fun `should convert synced list`() {
        ...
        val result = mapper.apply(valueOf("EUR", "10"), rates)
        result.values.apply {
    ...
    fun `should return empty if no base currency found`() {
        ...
        val convertedRates = mapper.apply(valueOf("EUR"), rates)
    ...
    fun `should return all zeros`() {
        ...
        val syncRates = mapper.apply(valueOf("EUR", "0"), rates)
        assertTrue(syncRates.values.all { BigDecimal.ZERO.compareTo(it.value) == 0 })

        val unsyncRates = mapper.apply(valueOf("USD", "0"), rates)
    ...
    fun `should place base currency from middle to top`() {
        ...
        val list = mapper.apply(valueOf("EUR"), rates).values
    ...
    fun `should not change order if base currency on top`() {
        ...
        val list = mapper.apply(valueOf("AUD"), rates).values
    ...
    fun `should move base from bottom to top`() {
        ...
        val list = mapper.apply(valueOf("RUB"), rates).values
